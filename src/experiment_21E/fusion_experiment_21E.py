...
from src.experiment_21E.config_experiment_21E import RESULTS_DIR_EXP21E, THRESHOLD
...
def run_fusion_evaluation_exp21E():
    predictions_path = RESULTS_DIR_EXP21E / "single_view_predictions_exp21E.csv"
    ...
    prob_front = pivot["prob_phone_front_locked_exp21"].to_numpy(dtype=float)
    prob_side = pivot["prob_phone_side_light_regularized"].to_numpy(dtype=float)
    ...
    fusion_metrics = {
        ...
        "front_checkpoint": "locked_exp21_stride30_best",
        "side_checkpoint": "exp21E_light_regularized_val_loss_monitor",
    }
    df_out = pivot.copy()
    ...
    (RESULTS_DIR_EXP21E / "fusion_metrics_exp21E.json").write_text(json.dumps(fusion_metrics, indent=2), encoding="utf-8")
    df_out.to_csv(RESULTS_DIR_EXP21E / "fusion_predictions_exp21E.csv", index=False)
    log.info("Saved Experiment 21E fusion metrics and predictions to %s", RESULTS_DIR_EXP21E)
    return fusion_metrics


def main():
    print(json.dumps(run_fusion_evaluation_exp21E(), indent=2))
