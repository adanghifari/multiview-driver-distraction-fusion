...
def build_markdown(summary: dict) -> str:
    front = summary["front_locked_exp21"]
    ...
    side_gap = diag["side"]["train_validation_loss_gap_at_best_epoch"]
    baseline_gap = REFERENCE_EXP21_STRIDE30_BEST["side_train_validation_loss_gap"]
    return "\n".join(
        [
            ...
            f"- Frame stride: {FRAME_STRIDE_EXP21E}.",
            ...
            f"- Side 21E: lr={SIDE_CONFIG['learning_rate']}, wd={SIDE_CONFIG['weight_decay']}, dropout={SIDE_CONFIG['dropout']}, LS={SIDE_CONFIG['label_smoothing']}, freeze={SIDE_CONFIG['num_stages_to_freeze']}, patience={SIDE_CONFIG['early_stopping_patience']}, monitor={SIDE_CONFIG['checkpoint_monitor']}.",
            "",
            "## Results",
            "",
            f"- Front locked Macro F1: {front['f1_macro']:.5f}; confusion: {front['confusion_matrix']}; status: {diag['front_locked_exp21']['generalization_status']}.",
            ...
            "",
            f"- Exp21 stride30-best side gap: {baseline_gap:.5f}.",
            f"- Exp21E side gap: {side_gap:.5f}.",
            f"- Exp21 stride30-best fusion Macro F1: {REFERENCE_EXP21_STRIDE30_BEST['average_fusion_f1_macro']:.5f}.",
            f"- Exp21D fusion Macro F1: {REFERENCE_EXP21D['average_fusion_f1_macro']:.5f}.",
...
def run_summary_exp21E():
    front = _read_json(RESULTS_DIR_EXP21E / "front_locked_exp21_metrics_exp21E.json")
    side = _read_json(RESULTS_DIR_EXP21E / "side_metrics_exp21E.json")
    fusion = _read_json(RESULTS_DIR_EXP21E / "fusion_metrics_exp21E.json")
    diagnostics = {
        "front_locked_exp21": _training_diag(FRONT_CONFIG_LOCKED["summary_path"]),
        "side": _training_diag(SIDE_CONFIG["summary_path"]),
    }
    rows = [
        {"method": "front_locked_exp21", **{k: front.get(k) for k in ["accuracy", "precision_macro", "recall_macro", "f1_macro", "safe_to_phone", "phone_to_safe", "ece_binary", "brier_score"]}, "generalization_status": diagnostics["front_locked_exp21"]["generalization_status"]},
        {"method": "side_single_view_21E", **{k: side.get(k) for k in ["accuracy", "precision_macro", "recall_macro", "f1_macro", "safe_to_phone", "phone_to_safe", "ece_binary", "brier_score"]}, "generalization_status": diagnostics["side"]["generalization_status"]},
    ...
    summary = {
        "experiment": "experiment_21E",
        "description": PIPELINE_DESCRIPTION,
        "front_locked_exp21": front,
        "side_single_view": side,
        "fusion": fusion,
        "training_diagnostics": diagnostics,
        "reference_exp21_stride30_best": REFERENCE_EXP21_STRIDE30_BEST,
        "reference_exp21D": REFERENCE_EXP21D,
        "reference_final_v1": REFERENCE_FINAL_V1,
        "reference_exp8": REFERENCE_EXP8,
    }
    pd.DataFrame(rows).to_csv(RESULTS_DIR_EXP21E / "experiment_21E_table.csv", index=False)
    (RESULTS_DIR_EXP21E / "experiment_21E_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (RESULTS_DIR_EXP21E / "experiment_21E_summary.md").write_text(build_markdown(summary), encoding="utf-8")
    return summary


def main():
    print(json.dumps(run_summary_exp21E(), indent=2))
