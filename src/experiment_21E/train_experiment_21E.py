...
def _class_weights_from_config() -> list[float]:
    ...
    if raw != "balanced":
        return raw
    df_train = load_split_dataframe("side", "train", frame_stride=FRAME_STRIDE_EXP21E)
    counts = {
...
def train_side_exp21E():
    seed_everything(SPLIT_SEED)
    CHECKPOINTS_DIR_EXP21E.mkdir(parents=True, exist_ok=True)
    RESULTS_DIR_EXP21E.mkdir(parents=True, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("Device: %s", device)
    loaders = get_all_dataloaders("side", frame_stride=FRAME_STRIDE_EXP21E)
    ...
    log.info("=" * 65)
    log.info(
        "Training Experiment 21E side | frame_stride=%d | checkpoint monitor=%s | patience=%d",
        FRAME_STRIDE_EXP21E,
    ...
    for epoch in range(1, MAX_EPOCHS_EXP21E + 1):
            ...
            torch.save(
                {
                    ...
                    "hyperparameters": {
                        ...
                        "class_weights": class_weights.cpu().tolist(),
                        "checkpoint_monitor": SIDE_CONFIG["checkpoint_monitor"],
                        "frame_stride": FRAME_STRIDE_EXP21E,
                    },
                },
...
def main():
    train_side_exp21E()
    print(json.dumps({"side_checkpoint": str(SIDE_CONFIG["checkpoint_path"]), "results_dir": str(RESULTS_DIR_EXP21E)}, indent=2))
