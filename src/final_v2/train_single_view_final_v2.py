...
def train_front_final_v2():
    """Train final_v2 front model using the Exp21B stabilization recipe."""
...
def _class_weights_from_config() -> list[float]:
    ...
    if raw != "balanced":
        return raw
    df_train = load_split_dataframe("side", "train", frame_stride=FRAME_STRIDE_FINAL)
    counts = {
...
def train_side_final_v2():
    """Train final_v2 side model using validation-loss checkpoint selection."""
    ...
    loaders = get_all_dataloaders("side", frame_stride=FRAME_STRIDE_FINAL)
    train_loader = loaders["train"]
    val_loader = loaders["val"]

    model = build_model(
        pretrained=SIDE_CONFIG["pretrained"],
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode="min",
        factor=SIDE_CONFIG["scheduler_factor"],
        patience=SIDE_CONFIG["scheduler_patience"],
    )
    ...
    log.info("=" * 65)
    log.info(
        "Training final_v2 side | frame_stride=%d | checkpoint monitor=%s | patience=%d",
        FRAME_STRIDE_FINAL,
        SIDE_CONFIG["checkpoint_monitor"],
        SIDE_CONFIG["early_stopping_patience"],
    )
    log.info("=" * 65)

    for epoch in range(1, SIDE_CONFIG["max_epochs"] + 1):
            ...
            torch.save(
                {
                    ...
                    "hyperparameters": {
                        ...
                        "lr_scheduler_patience": SIDE_CONFIG["scheduler_patience"],
                        "class_weights": class_weights.cpu().tolist(),
                        "checkpoint_monitor": SIDE_CONFIG["checkpoint_monitor"],
                        "frame_stride": FRAME_STRIDE_FINAL,
                    },
...
def read_best_training_snapshot(history_path, monitor: str = "val_macro_f1"):
...
def summarize_training_outputs():
    outputs = {}
    if FRONT_CONFIG["history_path"].exists():
        outputs["front"] = read_best_training_snapshot(FRONT_CONFIG["history_path"], "val_macro_f1")
    else:
        outputs["front"] = None
    if SIDE_CONFIG["history_path"].exists():
        outputs["side"] = read_best_training_snapshot(SIDE_CONFIG["history_path"], "val_loss")
    else:
        outputs["side"] = None
    return outputs
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train final_v2 single-view models.")
