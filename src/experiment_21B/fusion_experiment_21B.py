...
from src.experiment_21B.config_experiment_21B import RESULTS_DIR_EXP21B, THRESHOLD
...
def run_fusion_evaluation_exp21B():
    predictions_path = RESULTS_DIR_EXP21B / "single_view_predictions_exp21B.csv"
    ...
    prob_side = pivot["prob_phone_side_locked_exp21A"].to_numpy(dtype=float)
    ...
    (RESULTS_DIR_EXP21B / "fusion_metrics_exp21B.json").write_text(json.dumps(fusion_metrics, indent=2), encoding="utf-8")
    df_out.to_csv(RESULTS_DIR_EXP21B / "fusion_predictions_exp21B.csv", index=False)
    log.info("Saved Experiment 21B fusion metrics and predictions to %s", RESULTS_DIR_EXP21B)
    return fusion_metrics


def main():
    print(json.dumps(run_fusion_evaluation_exp21B(), indent=2))
