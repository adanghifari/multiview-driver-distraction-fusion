...
from src.experiment_21D.config_experiment_21D import (
    FRAME_STRIDE_EXP21D,
    FRONT_CONFIG_LOCKED,
...
def build_markdown(summary: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Frame stride: {FRAME_STRIDE_EXP21D}.",
            ...
            f"- Side 21D: lr={SIDE_CONFIG['learning_rate']}, wd={SIDE_CONFIG['weight_decay']}, dropout={SIDE_CONFIG['dropout']}, LS={SIDE_CONFIG['label_smoothing']}, freeze={SIDE_CONFIG['num_stages_to_freeze']}, patience={SIDE_CONFIG['early_stopping_patience']}, monitor={SIDE_CONFIG['checkpoint_monitor']}.",
            "",
            "## Results",
            "",
...
def run_summary_exp21D():
    front = _read_json(RESULTS_DIR_EXP21D / "front_locked_exp21_metrics_exp21D.json")
    side = _read_json(RESULTS_DIR_EXP21D / "side_metrics_exp21D.json")
    fusion = _read_json(RESULTS_DIR_EXP21D / "fusion_metrics_exp21D.json")
    diagnostics = {
    ...
    rows = [
        ...
        {"method": "side_single_view_21D", **{k: side.get(k) for k in ["accuracy", "precision_macro", "recall_macro", "f1_macro", "safe_to_phone", "phone_to_safe", "ece_binary", "brier_score"]}, "generalization_status": diagnostics["side"]["generalization_status"]},
    ...
    pd.DataFrame(rows).to_csv(RESULTS_DIR_EXP21D / "experiment_21D_table.csv", index=False)
    (RESULTS_DIR_EXP21D / "experiment_21D_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (RESULTS_DIR_EXP21D / "experiment_21D_summary.md").write_text(build_markdown(summary), encoding="utf-8")
    return summary


def main():
    print(json.dumps(run_summary_exp21D(), indent=2))
