...
def load_paired_split_dataframe_exp21D(split_name: str) -> pd.DataFrame:
    ...
    df = df[(df["frame"] - 1) % FRAME_STRIDE_EXP21D == 0].copy()
...
def get_paired_loader_exp21D(split_name: str) -> tuple[pd.DataFrame, DataLoader]:
    df = load_paired_split_dataframe_exp21D(split_name)
...
def run_single_view_evaluation_exp21D():
    RESULTS_DIR_EXP21D.mkdir(parents=True, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    df_test, loader = get_paired_loader_exp21D("test")
    ...
    assert_checkpoint_frame_stride(front_ckpt, str(FRONT_CONFIG_LOCKED["checkpoint_path"]), FRAME_STRIDE_EXP21D, "Exp21 locked front")
    assert_checkpoint_frame_stride(side_ckpt, str(SIDE_CONFIG["checkpoint_path"]), FRAME_STRIDE_EXP21D, "Exp21D side")
    ...
    for img_front, img_side, labels, indices in loader:
        ...
        for batch_idx in range(len(labels)):
            ...
            rows.append({**base, "view": "side_regularized", "prob_phone": round(float(probs_side[batch_idx]), 8), "pred_label": int(preds_side[batch_idx])})

    if indices_all != list(range(len(df_test))):
        raise AssertionError("Experiment 21D paired loader order is not synchronized with dataframe.")
    ...
    (RESULTS_DIR_EXP21D / "front_locked_exp21_metrics_exp21D.json").write_text(json.dumps(front_metrics, indent=2), encoding="utf-8")
    (RESULTS_DIR_EXP21D / "side_metrics_exp21D.json").write_text(json.dumps(side_metrics, indent=2), encoding="utf-8")
    pd.DataFrame(rows).to_csv(RESULTS_DIR_EXP21D / "single_view_predictions_exp21D.csv", index=False)
    log.info("Saved Experiment 21D single-view predictions and metrics to %s", RESULTS_DIR_EXP21D)
    return {"front_locked_metrics": front_metrics, "side_metrics": side_metrics}


def main():
    print(json.dumps(run_single_view_evaluation_exp21D(), indent=2))
