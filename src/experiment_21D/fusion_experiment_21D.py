...
from src.experiment_21D.config_experiment_21D import RESULTS_DIR_EXP21D, THRESHOLD
...
def run_fusion_evaluation_exp21D():
    predictions_path = RESULTS_DIR_EXP21D / "single_view_predictions_exp21D.csv"
    ...
    prob_side = pivot["prob_phone_side_regularized"].to_numpy(dtype=float)
    ...
    fusion_metrics = {
        ...
        "side_checkpoint": "exp21D_regularized_val_loss_monitor",
    }
    df_out = pivot.copy()
    ...
    (RESULTS_DIR_EXP21D / "fusion_metrics_exp21D.json").write_text(json.dumps(fusion_metrics, indent=2), encoding="utf-8")
    df_out.to_csv(RESULTS_DIR_EXP21D / "fusion_predictions_exp21D.csv", index=False)
    log.info("Saved Experiment 21D fusion metrics and predictions to %s", RESULTS_DIR_EXP21D)
    return fusion_metrics


def main():
    print(json.dumps(run_fusion_evaluation_exp21D(), indent=2))
