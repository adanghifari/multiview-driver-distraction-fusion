...
def load_paired_split_dataframe_exp21C(split_name: str) -> pd.DataFrame:
    ...
    df = df[(df["frame"] - 1) % FRAME_STRIDE_EXP21C == 0].copy()
...
def get_paired_loader_exp21C(split_name: str) -> tuple[pd.DataFrame, DataLoader]:
    df = load_paired_split_dataframe_exp21C(split_name)
...
def run_single_view_evaluation_exp21C():
    RESULTS_DIR_EXP21C.mkdir(parents=True, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    df_test, loader = get_paired_loader_exp21C("test")

    if not FRONT_CONFIG_LOCKED["checkpoint_path"].exists():
        raise FileNotFoundError(
            f"Locked Exp21B front checkpoint not found: {FRONT_CONFIG_LOCKED['checkpoint_path']}. "
            "Run Experiment 21B first or restore the checkpoint."
        )
    ...
    assert_checkpoint_frame_stride(front_ckpt, str(FRONT_CONFIG_LOCKED["checkpoint_path"]), FRAME_STRIDE_EXP21C, "Exp21B locked front")
    assert_checkpoint_frame_stride(side_ckpt, str(SIDE_CONFIG["checkpoint_path"]), FRAME_STRIDE_EXP21C, "Exp21C side")
    ...
    for img_front, img_side, labels, indices in loader:
        ...
        for batch_idx in range(len(labels)):
            ...
            rows.append({**base, "view": "front_locked_exp21B", "prob_phone": round(float(probs_front[batch_idx]), 8), "pred_label": int(preds_front[batch_idx])})
    ...
    if indices_all != list(range(len(df_test))):
        raise AssertionError("Experiment 21C paired loader order is not synchronized with dataframe.")
    ...
    (RESULTS_DIR_EXP21C / "front_locked_exp21B_metrics_exp21C.json").write_text(json.dumps(front_metrics, indent=2), encoding="utf-8")
    (RESULTS_DIR_EXP21C / "side_metrics_exp21C.json").write_text(json.dumps(side_metrics, indent=2), encoding="utf-8")
    pd.DataFrame(rows).to_csv(RESULTS_DIR_EXP21C / "single_view_predictions_exp21C.csv", index=False)
    log.info("Saved Experiment 21C single-view predictions and metrics to %s", RESULTS_DIR_EXP21C)
    return {"front_locked_metrics": front_metrics, "side_metrics": side_metrics}


def main():
    print(json.dumps(run_single_view_evaluation_exp21C(), indent=2))
