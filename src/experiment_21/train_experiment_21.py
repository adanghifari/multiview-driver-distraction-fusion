...
def _side_class_weights_from_config() -> list[float]:
    raw = SIDE_CONFIG["class_weights"]
    if raw != "balanced":
        return raw
    df_train = load_split_dataframe("side", "train", frame_stride=FRAME_STRIDE_EXP21)
    counts = {
...
def train_side_exp21():
    ...
    loaders = get_all_dataloaders("side", frame_stride=FRAME_STRIDE_EXP21)
    ...
    class_weights = torch.tensor(_side_class_weights_from_config(), dtype=torch.float).to(device)
    ...
    log.info("=" * 65)
    log.info(
        "Training Experiment 21 side | frame_stride=%d | checkpoint monitor=%s | patience=%d",
        FRAME_STRIDE_EXP21,
    ...
    for epoch in range(1, MAX_EPOCHS_EXP21 + 1):
