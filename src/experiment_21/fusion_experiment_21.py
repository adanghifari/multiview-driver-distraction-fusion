...
from src.experiment_21.config_experiment_21 import RESULTS_DIR_EXP21, THRESHOLD
...
def run_fusion_evaluation_exp21():
    predictions_path = RESULTS_DIR_EXP21 / "single_view_predictions_exp21.csv"
    ...
    (RESULTS_DIR_EXP21 / "fusion_metrics_exp21.json").write_text(json.dumps(fusion_metrics, indent=2), encoding="utf-8")
    df_out.to_csv(RESULTS_DIR_EXP21 / "fusion_predictions_exp21.csv", index=False)
    log.info("Saved Experiment 21 fusion metrics and predictions to %s", RESULTS_DIR_EXP21)
    return fusion_metrics


def main():
    print(json.dumps(run_fusion_evaluation_exp21(), indent=2))
